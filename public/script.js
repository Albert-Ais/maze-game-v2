...
// ---------------- QUIZ ----------------
function ask(i){
socket.emit("requestQuiz",{itemId:i.id});
}

socket.on("quizPopup",d=>{
let ans=prompt(d.q);
...
return Math.abs(player.x-x)<=VIEW&&Math.abs(player.y-y)<=VIEW;
}
...
camX += (player.x*tile - c.width/2 - camX)*0.15;
camY += (player.y*tile - c.height/2 - camY)*0.15;
